...
file_path = "../01Data/ZipCodeTotalDeathCount.csv"
deaths <- readr::read_csv(file_path)
names(deaths)

df <- plyr::rename(deaths, c("table"="tbl")) # table is a reserved word in Oracle so rename it to tbl.
names(df)
str(df) # Uncomment this line and  run just the lines to here to get column types to use for getting the list of measures.

measures <- c("Year", "ZIP Code", "Count")
#c("Year", "ZIP Code", "Causes of Death", "Count", "Location")
...
write.csv(df, gsub("ZipCodeTotalDeathCount", "ZipCodeTotalDeathCount", file_path), row.names=FALSE, na = "")
